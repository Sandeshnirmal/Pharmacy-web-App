...
export const addressAPI = {
  getAddresses: (userId) => {
    // Assuming the backend endpoint handles filtering by authenticated user
    // If not, userId would need to be passed as a parameter or in the URL
    return axiosInstance.get('/api/users/addresses/');
  },
...
export const courierAPI = {
  ...
  checkPincodeService: (pincode) => axiosInstance.get('/api/courier/shipments/check_pincode_serviceability/', { params: { pincode } }),
...
export const orderAPI = {
  ...
  createPendingOrder: (orderData) => axiosInstance.post('/api/order/pending/', orderData),
  finalizeOrderAfterPayment: (orderData) => axiosInstance.post('/api/order/enhanced/create-paid-order/', orderData),
};
...
export const cartAPI = {
  ...
  addItem: (productId, quantity = 1) => axiosInstance.post('/api/cart/add_item/', { product_id: productId, quantity }),
  updateItem: (productId, quantity) => axiosInstance.put('/api/cart/update_item/', { product_id: productId, quantity }),
  removeItem: (productId) => axiosInstance.delete('/api/cart/remove_item/', { data: { product_id: productId } }),
  clearCart: () => axiosInstance.post('/api/cart/clear_cart/'),
};
