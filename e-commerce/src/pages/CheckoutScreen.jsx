...
const CheckoutScreen = () => {
  ...
  const [currentBackendOrderId, setCurrentBackendOrderId] = useState(null);
  ...
  const handlePaymentSuccess = async (response) => {
    ...
    if (currentBackendOrderId) {
      await finalizeOrderAfterPayment({
        orderId: currentBackendOrderId,
  ...
  const initiateRazorpayPayment = async (orderId, amount) => {
    ...
    try {
      ...
      if (!razorpayOrderResponse.success) {
        toast.error(razorpayOrderResponse.error || 'Failed to create Razorpay order.');
        setIsPlacingOrder(false);
        return;
      }

      const razorpayOrderId = razorpayOrderResponse.razorpay_order_id;

      const options = {
        key: razorpayOrderResponse.key_id, // Key ID from backend response
        amount: razorpayOrderResponse.amount, // Amount from backend response (in paise)
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      if (!pendingOrderResponse.success) {
        toast.error(pendingOrderResponse.message || 'Failed to create pending order.');
        setIsPlacingOrder(false);
        return;
      }

      const backendOrderId = pendingOrderResponse.order_id;
      setCurrentBackendOrderId(backendOrderId);
      toast.info(pendingOrderResponse.message || 'Pending order created.');
  ...
  const finalizeOrderAfterPayment = async ({ orderId, paymentId, razorpayOrderId, razorpaySignature }) => {
    try {
      ...
      if (finalizationResponse.success) {
        clearCart();
        toast.success('Order placed successfully!');
        navigate(`/order-confirmation/${finalizationResponse.order_id}`);
      } else {
        toast.error(finalizationResponse.message || 'Failed to finalize order.');
      }
    } catch (error) {
